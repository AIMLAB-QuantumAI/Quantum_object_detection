...
from models.qcnn_utils_trial import create_model
...
if __name__ == '__main__':
    ...
    random_tensor = torch.rand(1,3,608,608)
    ...
    with torch.no_grad():
        for sample_idx in range(len(demo_dataset)):
            ...
            write_credit(out_img, (50, 410), org_fps=(900, 410),text_author='', fps=fps) #author remove

            if out_cap is None:
                out_cap_h, out_cap_w = out_img.shape[:2]
                fourcc = cv2.VideoWriter_fourcc(*'MJPG')
                out_path = os.path.join(configs.results_dir, 'demo file name')
