...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('/src/SFA3D/checkpoints/classic/ClassicalRPN/Model_classic_epoch_300.pth', map_location='cpu'))
    #model.load_state_dict(torch.load('/src/SFA3D/checkpoints/fpn_resnet_18/Model_block_epoch_20.pth', map_location='cpu'))
    ...
    with torch.no_grad():
        for sample_idx in range(len(demo_dataset)):
            ...
            write_credit(out_img, (50, 410), org_fps=(900, 410), fps=fps) #author remove

            if out_cap is None:
                out_cap_h, out_cap_w = out_img.shape[:2]
                fourcc = cv2.VideoWriter_fourcc(*'MJPG')
                out_path = os.path.join(configs.results_dir, 'classic_E300.avi')
